...
struct EmptyBoardTests {
    ...
    func fireAndMiss() async throws {
        ...
        game.addPlayer(board: board)
        game.addPlayer(board: BattleshipBoard(playerName: "Player 2"))

        let result = game.fire(at: Coordinate(x: .five, y: .B)) … å
        #expect(result).toEqual(.miss)

        let value = board.trackingBoardValue(at: Coordinate(x: .five, y: .B))
        #expect(value).toEqual(.miss)
    }
}
